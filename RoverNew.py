...
def turnRight(seconds):
    ...
    sendCommand(rightForward, halfSpeed)
    time.sleep(seconds)
...
def turnLeft(seconds):
    ...
    sendCommand(leftForward, halfSpeed)
    time.sleep(seconds)
    sendCommand(leftForward, zeroSpeed)
    sendCommand(rightForward, zeroSpeed)
...
for i in range(0, len(commands) - 1):
    ...
    degreesNeeded = bearing - currentBearing
    secondsToTurn = 0.2
    while (not(currentBearing < bearing + 7.5 and currentBearing > bearing - 7.5)):
        #Go
        distance = calculateDistance(diffLat, diffLon, sigma1, sigma2, R)
        #calculate bearing
        bearing = calculateBearing(lambda1, lambda2, sigma1, sigma2, distance, R)
